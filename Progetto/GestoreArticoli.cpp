...
bool GestoreArticoli::isPresente(Articolo A) const{
    for (auto a: articoli)
        if (a.getId() == A.getId())
            return false;
    return true;
...
const QList<QString> GestoreArticoli::keywordGuadagnoPiuAlto() const {
    ...
    for (auto art: articoli){
        ...
        for (auto k: keywordDisponibili)
            keywordPresenti(keyword, k);                                            //memorizza ogni keyword solo una volta
    }
    for (auto k: keyword){
        float prezzo = 0.0;
        for (auto art: articoli){
        ...
        prezzi.push_back(prezzo);                   //per ogni keyword viene memorizzato il guadagno
        if (prezzo > max){
            max = prezzo;
        }
    }
...
const QList<Articolo> GestoreArticoli::articoliAutoreOrdinatiPrezzo(int i) const {
    ...
    for (auto art: articoli){                                                        //mi salvo tutti gli articoli dell'autore i
        const QList<Autore>& autori = art.getAutoriInseriti();
        for (auto aut: autori){
            if (aut.getId() == i)
...
QList<QString> getStrutture(const QList<Autore>& autori) {               //memorizza ogni struttura una sola volta
...
const QList<QString> GestoreArticoli::strutturePiuProduttive(const QList<Autore>& autori) const {
    ...
    for (auto s: strutture){
        int prod = 0;                                                //si incrementa ogni volta che un articolo ha un autore di quella struttura
        for (auto art: articoli){
            int cont = 0;
            const QList<Autore>& autori = art.getAutoriInseriti();
            for (auto a: autori){
                ...
                if (cont > 0)         //serve per evitare che un'afferenza comune a più autori dello stesso articolo venga contata più di una volta
                    break;
            }
        }
